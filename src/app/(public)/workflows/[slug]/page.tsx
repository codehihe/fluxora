...
export default async function WorkflowDetailPage({ params }: { params: { slug: string } }) {
  ...
  const [userStatus, statsOffsets, relatedWorkflows, workflowJsonData] = await Promise.all([
    ...
    (async () => {
      ...
      const [savedWorkflow, vote, purchase] = await Promise.all([
  ...
  // Merge workflowJson (hide if locked)
  const workflowWithJson = { ...workflow, workflowJson: isLocked ? {} : (workflowJsonData?.workflowJson || {}) };
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="border-2 font-mono">
              ...
              <CardContent className="p-0">
                {isLocked ? (
                  <div className="flex flex-col items-center justify-center p-12 text-center bg-muted/20 min-h-[300px]">
                    <div className="bg-background w-16 h-16 rounded-full flex items-center justify-center mb-4 shadow">
                      <span className="text-2xl">🔒</span>
                    </div>
                    <h3 className="font-bold font-mono text-xl mb-2">Premium Workflow</h3>
                    <p className="text-muted-foreground font-mono max-w-sm">
                      Purchase this workflow to unlock the visualizer, setup guide, and workflow JSON.
                    </p>
                  </div>
                ) : (
                  <WorkflowVisualizer
            ...
            <Card className="border-2">
              ...
              <CardContent>
                {isLocked ? (
                  <div className="text-muted-foreground font-mono italic">Hidden for premium workflows</div>
                ) : (
